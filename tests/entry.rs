...
mod entry {
    ...
    fn creation_sets_status() {
        let entry = Entry::new(1, "[finished] my entry".to_string());
    ...
    fn creation_sets_content_from_context() {
        let entry = Entry::new(1, "[finished] my entry in context #666".to_string());
        assert_eq!(entry.content, "my entry in context");
    }

    #[test]
    fn creation_sets_color() {
        let entry = Entry::new(1, "[finished] my entry in context #666aaa".to_string());
        assert_eq!(entry.color.unwrap(), "666aaa");
    }

    #[test]
    fn creation_sets_no_color_when_none_was_provided() {
        let entry = Entry::new(1, "[finished] my entry in context".to_string());
